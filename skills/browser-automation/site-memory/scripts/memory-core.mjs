...
export function getRuntimeBase(runtimeBaseArg) {
  // 默认使用 site-memory 子技能下的 memory 目录
  const skillMemoryDir = join(dirname(fileURLToPath(import.meta.url)), '..', 'memory');
  const candidate = runtimeBaseArg || process.env.SITE_MEMORY_HOME || skillMemoryDir;
