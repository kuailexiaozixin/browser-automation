# web_tools.py — 布局感知的页面提取与带变化观测的 JS 执行
# 移植自 GenericAgent (lsdefine/GenericAgent) 的 simphtml.py + ga.py
# 遵循 MIT 许可证

import time, re, os

# ============================================================
# JS: 布局分析算法 — 覆盖检测 + 主次划分 + 浮动过滤
# 来自 GA simphtml.py js_optHTML (MIT)
# ============================================================
...
const _fc = [...domCopy.querySelectorAll('*')].filter(el => {
  ...
  .slice(0, 2);
_fc.forEach(el => { const r = getNodeInfo(el).rect; el.parentNode.removeChild(el); domCopy.appendChild(el); });
analyzeNode(domCopy); 
...
# ============================================================
# JS: 列表检测算法 — 找重复列表/表格并评分
# 来自 GA simphtml.py js_findMainList (MIT)
# ============================================================
...
# ============================================================
# JS: 临时文本监控 — 定期采样页面文本，检出动态新增内容
# 来自 GA simphtml.py temp_monitor_js (MIT)
# ============================================================
...
# ============================================================
# Python: HTML 属性级压缩 — 移除冗余属性和长内容
# 来自 GA simphtml.py optimize_html_for_tokens (MIT)
# ============================================================
def optimize_html_for_tokens(html):
    """移除 SVG、style、长 URL、非白名单属性等，压缩 HTML token 消耗"""
...
# ============================================================
# Python: DOM 差异检测 — 比较执行前后的 HTML 变化
# 来自 GA simphtml.py find_changed_elements (MIT)
# ============================================================
def find_changed_elements(before_html, after_html):
    """比较前后两份 HTML，返回变化边界上的最大元素"""
    from bs4 import BeautifulSoup
...
# ============================================================
# Python: 递归预算截断 — 超出 token 预算时智能裁减
# 来自 GA simphtml.py smart_truncate (MIT)
# ============================================================
...
# ============================================================
# Python: 浏览器端执行布局分析主函数
# 来自 GA simphtml.py get_main_block (MIT)
# ============================================================
def get_main_block(driver, extra_js="", text_only=False):
    """在浏览器中执行 js_optHTML 布局分析，返回简化 HTML 或纯文本"""
    # 注意：使用 CDP Runtime.evaluate 时 JS 顶层不支持 return 关键字，
    # 因此这里直接调用 optHTML（其返回值即 evaluate 表达式的结果），
    # 无需 return。各驱动层无需额外适配。
    result = driver.execute_js(f"{extra_js}\n{js_optHTML}\noptHTML({str(text_only).lower()});")
    page = result.get('data', result.get('result', ''))
    if text_only:
...
# ============================================================
# Python: 获取简化页面 HTML（含列表截断）
# 来自 GA simphtml.py get_html (MIT)
# ============================================================
def get_html(driver, cutlist=False, maxchars=35000, instruction="", extra_js="", text_only=False):
    """获取简化后的页面 HTML，可选列表截断和预算控制"""
    from bs4 import BeautifulSoup

    if cutlist:
        rr_result = driver.execute_js(js_findMainList + "findMainList(document.body);")
        rr = rr_result.get('data', rr_result.get('result', []))
...
# ============================================================
# Python: 获取瞬变文本
# 来自 GA simphtml.py start_temp_monitor + get_temp_texts (MIT)
# ============================================================
...
def get_temp_texts(driver):
    ...
    """
    try:
        r = driver.execute_js(js)
        return list(set(r.get('data', r.get('result', []))))
    except Exception as e:
        return []


# ============================================================
# Python: 带页面变化观测的 JS 执行器
# 来自 GA simphtml.py execute_js_rich (MIT)
# ============================================================
def execute_js_rich(script, driver, no_monitor=False):
    """执行 JS 并观测页面变化，返回结构化结果"""
    last_html = None
    if not no_monitor:
        try:
    ...
    before_sids = set()
    try:
        if hasattr(driver, 'get_session_dict'):
    ...
    try:
        ...
        result = response.get('data') if 'data' in response else response.get('result')
    ...
    if hasattr(driver, 'default_session_id'):
        rr["tab_id"] = driver.default_session_id

    if reloaded:
        rr['reloaded'] = reloaded
    if response.get('newTabs'):
        rr['newTabs'] = response['newTabs']
    elif hasattr(driver, 'get_session_dict'):
        try:
...
# ============================================================
# Python: 错误格式化 — 来自 GA ga.py format_error (MIT)
# ============================================================
def format_error(e):
    """格式化异常信息为简短字符串"""
    ...
    if tb:
        ...
        return f"{exc_type.__name__}: {str(e)} @ {fname}:{f.lineno}, {f.name}"
    return f"{exc_type.__name__}: {str(e)}"


# ============================================================
# 公共 API: web_scan — 布局感知的页面内容提取
# 来自 GA ga.py web_scan (MIT)
# ============================================================
def web_scan(driver, tabs_only=False, switch_tab_id=None, text_only=False, maxlen=35000):
    """获取当前页面的简化HTML内容和标签页列表。
    
    参数:
        driver: 支持 execute_js() 和 get_all_sessions()/default_session_id 的浏览器驱动对象
        tabs_only: 仅返回标签页列表，不获取HTML内容
        switch_tab_id: 切换到指定标签页后再扫描
        text_only: 只返回纯文本（不返回HTML）
        maxlen: 返回内容的最大字符数
    
    返回:
        dict: {status, metadata(tabs, active_tab), [content]}
    """
    try:
        # 获取标签页列表
        sessions = []
        if hasattr(driver, 'get_all_sessions'):
            sessions = driver.get_all_sessions()
        elif hasattr(driver, 'get_session_dict'):
            sdict = driver.get_session_dict()
            sessions = [{'id': k, 'url': v} for k, v in sdict.items()]
        ...
        for sess in sessions:
            sess_copy = dict(sess)
            sess_copy.pop('connected_at', None)
            sess_copy.pop('type', None)
            url = sess_copy.get('url', '')
            if len(url) > 50:
                sess_copy['url'] = url[:50] + "..."
            tabs.append(sess_copy)

        if switch_tab_id and hasattr(driver, 'default_session_id'):
        ...
        if hasattr(driver, 'default_session_id'):
            result["metadata"]["active_tab"] = driver.default_session_id

        if not tabs_only:
            content = get_html(driver, cutlist=True, maxchars=maxlen, text_only=text_only)
            if text_only:
                content = _smart_format(content, max_str_len=maxlen // 3, omit_str='\n\n[omitted long content]\n\n')
...
# ============================================================
# 公共 API: web_execute — 带变化观测的 JS 执行
# 来自 GA ga.py web_execute_js (MIT)
# ============================================================
def web_execute_js(driver, script, switch_tab_id=None, no_monitor=False):
    """执行 JS 并返回结果 + 页面变化观测。
    
    参数:
        driver: 支持 execute_js() 的浏览器驱动对象
        script: 要执行的 JavaScript 代码
        switch_tab_id: 切换到指定标签页执行
        no_monitor: 跳过页面变化监控（省2-3秒，仅纯读取时设置）
    
    返回:
        dict: {status, js_return, tab_id, [newTabs], [transients], [diff], [error]}
    """
    try:
        if hasattr(driver, 'get_all_sessions'):
            sessions = driver.get_all_sessions()
        elif hasattr(driver, 'get_session_dict'):
            sessions = driver.get_session_dict()
        else:
            sessions = []
...
def _smart_format(text, max_str_len=3000, omit_str='...'):
    """简单的字符串截断替代 smart_format"""
    if text is None:
        return ''
    text = str(text)
    if len(text) <= max_str_len:
        return text
    return text[:max_str_len] + omit_str
